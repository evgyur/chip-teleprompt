...
`src/model.rs` — логика прокрутки и очистки текста; `src/main.rs` — Win32/GDI окно и ввод; `src/text_renderer.rs` — DirectWrite/Direct2D с дробным вертикальным смещением; `src/frame_clock.rs` — ожидание обновления монитора через DXGI; `src/tray.rs` — Windows Shell tray; `assets/app.ico` — многоразмерная иконка, встроенная в EXE. Программа использует только системные Windows DLL. Пользовательские сценарии не отправляются по сети; внешние ссылки открываются только по нажатию.

В версии 0.2.2 текст прокручивается без округления положения до целых пикселей. Кадры согласуются с монитором окна; при паузе поток ожидания спит. Если драйвер не предоставляет синхронизацию, используется ограниченный таймер без изменения системной частоты таймеров. Для редких несовместимых шрифтов предусмотрена GDI-отрисовка.

Измерение кадров на тестовом тексте: `Chip-Teleprompt.exe --measure-scroll C:\temp\teleprompt-timing 28`. Последнее число — положение ползунка скорости (0–100, 28 соответствует 17 px/s). Через три секунды тест закрывается и сохраняет интервалы и координаты. Пользовательский текст не записывается.
